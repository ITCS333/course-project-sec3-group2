...
function createUser($db, $data) {
    ...
    $check->execute([$email]);
    if ($check->fetch()) {
        sendResponse("Email already registered", 409);
    }
   ...
   $stmt = $db->prepare("INSERT INTO users (name, email, password, is_admin) VALUES (?, ?, ?, ?)");
    ...
    if ($stmt->execute([$name, $email, $hashedPassword, $is_admin])) {
...
function updateUser($db, $data) {
    ...
    //       (name, email, is_admin). Skip any field not present in $data.
        $fields = [];
    $params = [];
    if (isset($data['name'])) { $fields[] = "name = ?"; $params[] = sanitizeInput($data['name']); }
    if (isset($data['is_admin'])) { $fields[] = "is_admin = ?"; $params[] = (int)$data['is_admin']; }
    ...
    if (isset($data['email'])) {
        ...
        if ($dup->fetch()) { sendResponse("Email in use", 409); }
        $fields[] = "email = ?"; $params[] = $email;
    }
    if (empty($fields)) { sendResponse("No changes provided", 400); }
...
function changePassword($db, $data) {
    ...
    if (empty($data['id']) || empty($data['current_password']) || empty($data['new_password'])) {
        sendResponse("Missing password information", 400);
    }
    $id = (int)$data['id'];
